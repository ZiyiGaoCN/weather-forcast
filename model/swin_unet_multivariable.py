...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        relative_position_bias = self.relative_position_bias_table[self.relative_position_index.view(-1)].view(
            self.window_size[0] * self.window_size[1], self.window_size[2], self.window_size[0] * self.window_size[1] * self.window_size[0]*self.window_size[1]*self.window_size[2], -1)  # Wh*Ww,Wh*Ww,nH
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        shifted_x = window_reverse_3D(attn_windows, self.window_size, H, W)  # B H' W' C
...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self, 
        ...
        self.out_chans = out_seq_length * output_dim
        ...
        for i_layer in range(self.num_layers):
            concat_linear = nn.Linear(2*int(self.hidden_dim*2**(self.num_layers-1-i_layer)),
            int(self.hidden_dim*2**(self.num_layers-1-i_layer))) if i_layer > 0 else nn.Identity()
            if i_layer ==0 :
                layer_up = PatchExpand(input_resolution=(patches_resolution[0] // (2 ** (self.num_layers-1-i_layer)),
                patches_resolution[1] // (2 ** (self.num_layers-1-i_layer))), dim=int(self.hidden_dim * 2 ** (self.num_layers-1-i_layer)), dim_scale=2, norm_layer=norm_layer)
            else:
                layer_up = BasicLayer_up(dim=int(self.hidden_dim * 2 ** (self.num_layers-1-i_layer)),
        ...
        self.norm_up= norm_layer(self.hidden_dim)
...
class WindowAttention_3D(nn.Module):
    ...
    def __init__(self, dim, window_size ,num_heads,  
        ...
        # define a parameter table of relative position bias
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1) * (2 * window_size[2] -1 if no_relative_z == False else 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH

        # get pair-wise relative position index for each token inside the window
        coords_h = torch.arange(self.window_size[0])
        coords_w = torch.arange(self.window_size[1])
        coords_z = torch.arange(self.window_size[2])
        coords = torch.stack(torch.meshgrid([coords_h, coords_w,coords_z]))  # 3, Wh, Ww, Wz
    ...
    def forward(self, x, mask=None):
        ...
        assert N == self.window_size[0] * self.window_size[1] * self.window_size[2], \
...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self.patch_embed = PatchEmbed_3D(
            lat_dim=lat_dim,lon_dim=lon_dim,z_dim=z_dim, patch_size=patch_size, embed_dim=hidden_dim,
            in_chans=C_dim,
            norm_layer=norm_layer,
            extra_dim=0)
        self.hidden_dim = hidden_dim
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer_3D(dim=int(self.hidden_dim * 2 ** i_layer),
                               input_resolution=(patches_resolution[0] // (2 ** i_layer), # 16, 8, 4, 2
                                                 patches_resolution[1] // (2 ** i_layer),
                                                 (patches_resolution[2]-1) // (2 ** i_layer)+1),
                               depth=depths[i_layer], # 2, 2, 2, 2
                               num_heads=num_heads[i_layer], # 3, 6, 12, 24
    ...
    def forward(self, x):
        ...
        x = x.view(x.shape[0],self.patch_embed.patches_resolution[0],self.patch_embed.patches_resolution[1],-1,self.hidden_dim)
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer_3D(dim=int(self.hidden_dim * 2 ** (self.num_layers-i_layer)),
...
class Fusion(nn.Module):
    def __init__(self, 
        ...
        if norm_layer is not None:
            ...
            self.norm = None
        self.proj_out = nn.Linear(embed_dim, out_chans)
    ...
    def forward(self, x, timestep = None):
        ...
        if self.time_embed:
            t = timestep_embedding(timestep,dim=self.embed_dim).to(x.device)
...
class Model(nn.Module):
    def __init__(self,
        ...
        self.time_embed = time_embed
        self.split_range = [
            [((self.variable_dict[k]-1)//int(2**i)+1) for k in self.variable_order]
            for i in range(3)
        ]
        self.uncertainty_loss = uncertainty_loss 
    ...
    def forward(self, x, timestep = None):
        # [B,73,32,64]
        x = x.permute(0,2,3,1).contiguous().unsqueeze(-2) # [B,32,64,1,73] 
        x_varibles = {}
        acc = 0
    ...
    def flops(self,x):
        ...
        for k in self.variable_order:
            v = self.variable_dict[k]
            x_varibles[k] ,encoder_flops = self.encoders[k].flops(x[:,:,:,:,slice(acc,acc+v)]) # [B,32,64,1,13] in [B,32,64,1,96] out
            acc+=v
        ...
        for i,fusion in enumerate(self.fusions):
            x_downsample[i], fusion_flops =fusion.flops(x_downsample[i])
            flops += fusion_flops
        for i in range(3):
            x_downsample[i] = torch.split(x_downsample[i],[1 for _ in self.variable_order],dim=-2)
        x_upsample_varibles = {
        ...
        final = {}
        for k,decoder in self.decoders.items():
            final[k], decoder_flops=decoder.flops(x_upsample_varibles[k])
            flops += decoder_flops
        final = torch.cat([final[k] for k in self.variable_order],dim=1)
        # final = final.squeeze(1).permute(0,3,1,2).contiguous()
        final.squeeze_(-1)
