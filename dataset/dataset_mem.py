...
def split_dataset(data_dir, ratio=0.8, transform=None, target_transform=None):
    ...
    train_data = WeatherDataet(load_dataset(data_dir, 2007, 2011).x, transform, target_transform, data_dir=data_dir,is_train=True)
    test_data = WeatherDataet(load_dataset(data_dir, 2011, 2012).x, transform, target_transform, data_dir=data_dir,is_train=False)
